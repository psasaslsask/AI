...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        if len(foodList) > 0:
            distances = [manhattanDistance(newPos, f) for f in foodList]
            score += 4.0 / min(distances)
        ...
        # Return your improved evaluation
        if score is None:
            return successorGameState.getScore()
        return score
        ...
        def minimax(state, depth, agentIndex):
            ...
            if agentIndex == 0:
                ...
                for action in legalActions:
                    ...
                    scores.append(minimax(successor, nextDepth, nextAgent))
                return max(scores)
            else:
                # Min node (Ghosts)
                scores = []
    ...
    def getAction(self, gameState: GameState):
        ...
        def expectimax(state, depth, agentIndex):
            ...
            if agentIndex == 0:
                ...
                for action in legal:
                    successor = state.generateSuccessor(agentIndex, action)
                    scores.append(expectimax(successor, nextDepth, nextAgent))
                return max(scores)
            else:
                # Chance node
                values = []
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    if len(foodList) > 0:
        distToFood = min(manhattanDistance(pos, f) for f in foodList)
        score += 4.0 / distToFood

    # Capsule distance
    if len(capsules) > 0:
        distToCaps = min(manhattanDistance(pos, c) for c in capsules)
        score += 2.0 / (distToCaps + 1)
    ...
    # Encourage eating remaining food
